...
export const getAnimeData = action({
  ...
  handler: async (_, args) => {
    ...
    console.log(apiKey);
    const data = await fetch(`https://api.myanimelist.net/v2/anime/${args.id}?fields=id,title,main_picture,alternative_titles,start_date,end_date,synopsis,mean,rank,popularity,num_list_users,num_scoring_users,nsfw,created_at,updated_at,media_type,status,genres,my_list_status,num_episodes,start_season,broadcast,source,average_episode_duration,rating,pictures,background,related_anime,related_manga,recommendations,studios,statistics`, {headers});
    const obj = await data.json()
    return obj;
  },
});
